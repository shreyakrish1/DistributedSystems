...
public class BrokerController {
    ...
    private HashMap<String, List<EventData>> publisherEventDataMap = new HashMap<>();
    ...
    */
    ...
    public ResponseEntity<HttpStatus> pushEvent(@RequestBody EventData eventData){
        ...
        LocalDateTime timestamp=eventData.getTimestamp();
        lock.lock();
        try{
            if(!publisherEventDataMap.containsKey(publisherId))
            {
                publisherEventDataMap.put(publisherId,new ArrayList<>());
            }
            List<EventData> events=publisherEventDataMap.get(publisherId);
            events.add(eventData);
            System.out.println("Event data with id: "+eventId+" received by the broker at Timestamp: "+timestamp);
            return ResponseEntity.ok(HttpStatus.OK);
            }  catch (Exception e) {
                e.printStackTrace();
                return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
